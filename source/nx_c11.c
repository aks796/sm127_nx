...
uint64_t sm127_c11_mtx_live, sm127_c11_cnd_live;
...
int mtx_init(mtx_t *mtx, int type) {
  ...
  if (r == NX_THRD_SUCCESS) sm127_c11_mtx_live++;
  return r;
}
...
void mtx_destroy(mtx_t *mtx) {
  ...
  s->in_use = 0;
  mutexUnlock(&g_pool_lk);
  if (sm127_c11_mtx_live) sm127_c11_mtx_live--;
...
void cnd_destroy(cnd_t *cnd) {
  ...
  s->in_use = 0;
  mutexUnlock(&g_pool_lk);
  if (sm127_c11_cnd_live) sm127_c11_cnd_live--;
  const uint32_t zero = 0;
...
void sm127_c11_report(void) {
  ...
  debugPrintf("[c11] shim active: %llu mutex(es), %llu condvar(s) live\n",
              (unsigned long long)sm127_c11_mtx_live,
              (unsigned long long)sm127_c11_cnd_live);
}
