...
void so_finalize(so_module *mod) {
  ...
  // ------------------------------------------------------------------------
  // Re-point the symbol tables from load_base to load_virtbase.
  //
  // MapProcessCodeMemory makes the SOURCE range inaccessible -- the kernel
  // drops load_base to Perm_None once it is mapped at load_virtbase. syms and
  // dynstrtab were computed as load_base + sh_addr in so_load, so from here on
  // every read through them is a Data Abort, not stale data.
  //
  // That matters because so_resolve_symbol walks the whole so_list and calls
  // so_lookup_export on each module, INCLUDING already-finalized ones. Any
  // module loaded later that needs a symbol not in the shim table -- e.g.
  // libfmod.so reaching for C++ symbols in libc++_shared -- walks straight
  // into a finalized module and faults on mod->syms[i].
  //
  // .dynsym and .dynstr are SHF_ALLOC, so their contents are inside a PT_LOAD
  // segment and exist at load_virtbase + sh_addr just as they did at
  // load_base + sh_addr. Rebasing the two pointers costs nothing and keeps
  // cross-module lookup working for the lifetime of the process.
  //
  // This is the crash from the first hardware run:
  //   so_lookup_export (so_util.c:323) <- so_resolve_symbol
  //   <- sm127_resolve_imports <- sm127_load_fmod_module <- dlopen_fake
  // ------------------------------------------------------------------------
  if (mod->syms)
...
// Which module owns this address, if any. Walks the same so_list every other
// lookup uses, so it automatically covers every module the port loads --
// including ones added later, which a hand-maintained registry would miss.
// Used by the crash handler to turn a raw PC into <module>+0x<offset>.
// Head of the loaded-module list, for callers that want to enumerate rather
// than look up (the crash handler prints a module map; bases move every boot,
// so an offset is only meaningful next to the base it came from).
so_module *so_get_list(void) { return so_list; }
...
so_module *so_find_module_by_addr(const void *addr) {
  ...
  for (so_module *m = so_list; m; m = m->next) {
    ...
    if (base && a >= base && a < base + m->load_size)
      return m;
  }
  return NULL;
}
...
static uintptr_t so_lookup_export(so_module *mod, const char *name) {
  // Both must be live. Before so_finalize they point into load_base; after it
  // they are rebased onto load_virtbase, because MapProcessCodeMemory makes
  // load_base inaccessible. If either is ever null here, something skipped
  // that rebase and the next line would be a Data Abort with no explanation.
  if (!mod->syms || !mod->dynstrtab || mod->num_syms <= 0)
    return 0;
