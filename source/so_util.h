...
typedef struct so_module {
  ...
  // Set by so_finalize. MapProcessCodeMemory makes load_base inaccessible, so
  // syms/dynstrtab are rebased onto load_virtbase there; this records that it
  // happened. Purely diagnostic -- nothing branches on it.
  int finalized;
  char *shstrtab;
  char *dynstrtab;
} so_module;
...
// Which loaded module contains this address, or NULL. See so_util.c.
so_module *so_find_module_by_addr(const void *addr);

// Head of the loaded-module list. See so_util.c.
so_module *so_get_list(void);
