...
// rename() that REPLACES an existing destination, as POSIX does and FAT does
// not. Used by both rename paths -- libc (rename_fake) and, more importantly,
// the JNI DirectoryAccessHandler in jni_fake.c, which is the one Godot's
// Android build actually uses for user:// paths.
