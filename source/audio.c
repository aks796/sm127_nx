...
static int s_sink_client = -1;
static Mutex s_audout_lock;

// ---------------------------------------------------------------------------
// This used to own audout directly: audoutInitialize + audoutStartAudioOut,
// its own AudioOutBuffer pool, AppendAudioOutBuffer / GetReleased / WaitPlayFinish.
//
// audout is single-instance, and on the first hardware run Godot's OpenSL
// driver got here first and took it -- so when FMOD came up moments later and
// asked AAudio for a stream, audio_sink's audoutStartAudioOut returned 0x499
// (already open). FMOD reported "Error initializing output device" and every
// later FMOD call failed with an invalid handle.
//
// docs/PORT_NOTES.md flagged this ("audio device ownership ... NOT yet
// enforced") and it went unenforced. audio_sink is multi-client and mixing
// precisely for this case, so this path is now a client of it like any other.
// The conversion machinery below is unchanged; only the destination moved.
// ---------------------------------------------------------------------------

static void audout_ensure(void) {
  mutexLock(&s_audout_lock);
  if (!s_audout_ready) {
    s_sink_client = audio_sink_client_open("godot-opensl");
    if (s_sink_client >= 0) {
      s_audout_ready = 1;
      debugPrintf("[audio] sink client %d opened (%d Hz, %d ch)\n",
                  s_sink_client, SINK_SAMPLE_RATE, SINK_CHANNELS);
    } else {
      debugPrintf("[audio] could not open a sink client\n");
    }
  }
...
// free_one() and any_in_flight() are gone with the audout ownership: the sink
// copies on write, so no buffer is ever outstanding from this side and there
// is nothing to reclaim or wait on. Left out rather than left unused, since
// -Wunused-function would fail the build.
// find_free() is gone too. The buffer POOL existed because audout held
// buffers asynchronously; audio_sink_client_write copies and returns, so one
// scratch buffer is enough and only this thread ever touches it.
//
// Worth removing rather than leaving harmless: if find_free() had ever
// returned NULL, the loop below would have fallen through to a condvar wait
// whose predicate is already false when there is queued data -- a busy spin
// with no way to notice it except a hot core.
static void feeder(void *arg) {
  ...
  for (;;) {
    ...
    OutBuf *b = (have && s_audout_ready) ? &p->bufs[0] : NULL;
    if (have && (b || !s_audout_ready)) {
      ...
      if (b) {
        ...
        if (frames) {
          // Blocking write. The sink backpressures against its feeder, which
          // audout paces -- the same real-time pacing the old
          // AppendAudioOutBuffer path got from the device, without owning it.
          audio_sink_client_write(s_sink_client, (const int16_t *)b->mem,
                                  (int)frames, SINK_CHANNELS);

          static int logged_first = 0;
          if (!logged_first) { logged_first = 1; debugPrintf("[audio] streaming to sink (%u frames/buf)\n", (unsigned)frames); }
        }
      }
    ...
    // The old code waited on audout here when buffers were outstanding. The
    // blocking write above already provides that pacing, and this thread no
    // longer owns the device, so there is nothing to wait on.
...
static SLresult player_Realize(void *self, SLboolean async) {
  ...
  if (!p->realized) {
    ...
    mutexInit(&p->lock);
    condvarInit(&p->cond);
    // Core 2: this thread runs Godot's audio mixing (the buffer-queue
    // callback), which must not share core 1 with the frame thread. Falls back
    // to the default core if core 2 is not available to this process.
    Result trc = threadCreate(&p->thread, feeder, p, NULL, 64 * 1024, 0x2C, 2);
    if (R_FAILED(trc)) trc = threadCreate(&p->thread, feeder, p, NULL, 64 * 1024, 0x2C, -2);
    if (R_SUCCEEDED(trc)) {
