...
// What the runtime tried to install for a signal, or NULL. Nothing delivers to
// it yet; the crash handler reports its presence so the gap is visible rather
// than inferred. See sigaction_fake.
void *sm127_recorded_signal_handler(int sig);

// The sa_flags the runtime asked for. SA_SIGINFO (4) means a 3-argument
// handler -- (int, siginfo_t*, void*) -- which is what any future attempt at
// delivering a fault would have to construct arguments for.
...
// chdir with the same sandbox_path() rebase every other path call gets.
// See libc_shim.c -- its absence broke the shader cache.
...
// O_CREAT as the ENGINE spells it. Any shim testing for "create" must use
// this, not newlib's O_CREAT -- the two differ and newlib's value is bionic's
// O_TRUNC. See libc_shim.c.
#define BIONIC_O_CREAT 0x000040
...
// Horizon csrng behind the two POSIX RNG entry points. See libc_shim.c.
// 1 if this is the synthetic /dev/urandom descriptor. See libc_shim.c.
int sm127_is_urandom_fd(int fd);
